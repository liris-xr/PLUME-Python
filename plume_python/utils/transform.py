...
class Transform:
    ...
    def set_local_rotation(self, local_rotation: quaternion):
        self._local_R_mtx[0:3, 0:3] = quaternion.as_rotation_matrix(
            local_rotation
        )
...
class TimestampedTransform:
    ...
    def get_world_rotation(self) -> quaternion:
        return quaternion.from_rotation_matrix(
            self.local_to_world_mtx[0:3, 0:3]
        )
...
def compute_transforms_time_series(
    record: Record, included_guids: set[str] = None
    ...
    update_samples: dict[
        int, list[FrameDataSample[transform_pb2.TransformUpdate]]
    ] = {}
    ...
    for frame in tqdm(record.frames_info, desc="Computing world positions"):
        ...
        if frame.frame_number in update_samples:
            for update_sample in update_samples[frame.frame_number]:
                ...
                if update_sample.payload.HasField("local_position"):
                    ...
                    local_transform.set_local_position(
                        np.array(
                            [
                                local_position.x,
                                local_position.y,
                ...
                if update_sample.payload.HasField("parent_transform_id"):
                    parent_guid = (
                        update_sample.payload.parent_transform_id.component_id
                    )
                    if (
                        parent_guid == "00000000000000000000000000000000"
                    ):  # null guid
                        local_transform.set_parent(None)
                    elif parent_guid in current_transforms:
                        local_transform.set_parent(
                            current_transforms[parent_guid]
                        )
                    else:
        ...
        for t in included_transforms:
            ...
            result.setdefault(
                t.get_guid(), list[TimestampedTransform]()
            ).append(timestamped_transform)
